...
RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  ...
  WHERE m.conversation_id = _conversation_id
    AND m.to_id = auth.uid()
    AND m.read_at IS NULL;

  UPDATE public.notifications n
  SET read_at = now()
  WHERE n.conversation_id = _conversation_id
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      WHERE c.id = conversation_id
        AND ((c.participant_a = from_id AND c.participant_b = to_id)
          OR (c.participant_b = from_id AND c.participant_a = to_id))
    )
  );
