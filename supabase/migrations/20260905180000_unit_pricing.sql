...
-- Database-authoritative transaction canonicalization. Buyer-supplied amount,
-- seller identity, quantity, pricing unit and commission are never trusted.
...
-- Pricing semantics become immutable once transaction history exists for a
-- listing, preventing historical orders from being reinterpreted.
...
-- Inventory is consumed only when an administrator verifies payment. The
-- transaction and listing are locked so concurrent verification cannot oversell.
...
-- The admin delivery override must never bypass payment verification for a
-- unit-priced transaction.
...
-- Unit-priced listings remain active while verified stock remains. Legacy
-- listings retain their original completion behavior.
...
-- Keep the public-store RPC aligned with the canonical pricing field.
