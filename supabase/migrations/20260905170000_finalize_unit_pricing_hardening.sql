...
-- Final forward-only hardening for explicit listing / each / pair pricing.
-- This migration intentionally does not rewrite or delete prior migrations.
-- Existing listings default to legacy whole-listing pricing, and historical
-- transaction amounts are preserved.
...
-- Preserve the full current buyer-eligibility/security boundary while making
-- the database authoritative for unit, quantity, amount and commission.
...
begin
  ...
  if total_numeric > 9223372036854775807 or total_numeric < -9223372036854775808 then
    raise exception 'The calculated transaction amount is outside the supported range';
  end if;
  ...
  -- Legacy listings preserve the established lifecycle. Unit-priced orders
  -- require explicit administrator payment verification before escrow funding.
...
-- Sellers may change pricing on a listing that has never had a transaction,
-- but pricing semantics become immutable once transaction history exists.
...
-- Only verified unit-priced purchases consume inventory. The transaction and
-- listing are locked together so competing verification attempts cannot
-- oversell the remaining quantity.
...
begin
  ...
  if tx_row.pricing_unit in ('each', 'pair') then
    ...
    if tx_row.quantity_purchased < 1 then
      raise exception 'Transaction quantity is invalid';
    end if;
...
-- Delivery override remains an administrator-only emergency tool, but it may
-- never bypass payment verification for a unit-priced order.
...
begin
  ...
  from public.transactions
  where id = _transaction_id
    and status not in ('Ready for Admin Payout','Seller Paid','Refunded to Buyer','Disputed')
  for update;

  if not found then
    raise exception 'Order is not active';
  end if;
  if tx_row.status = 'Pending Verification' then
    raise exception 'Payment must be verified before delivery can be marked';
  end if;
  ...
  if tx_row.listing_id is not null then
    if coalesce(tx_row.pricing_unit, 'listing') = 'listing' then
      update public.listings set is_active = false where id = tx_row.listing_id;
    else
      update public.listings set is_active = (batch_quantity > 0) where id = tx_row.listing_id;
    end if;
  end if;
end;
$$;
...
-- Unit-priced listings remain active after buyer receipt while verified stock
-- remains. Legacy listings retain their original completion behavior.
...
begin
  ...
  select verification_pin, listing_id, pricing_unit
    into pin, listing, unit
  from public.transactions
  where id = _transaction_id
...
-- Keep the public store response aligned with the canonical pricing field.
...
as $$
  ...
  select r.user_id, r.username, r.real_name, r.loft_name, r.home_state, r.avatar_url, r.is_verified_seller, r.is_online,
    coalesce((
      select jsonb_agg(to_jsonb(x) order by x.is_featured desc, x.is_verified_seller desc, x.creation_timestamp desc)
      from (
        select l.id, l.slug, l.category_type, l.breeder_id, l.custom_bird_name, l.breed_type, l.gender,
               l.price_ngn, l.pricing_unit, l.images, l.pedigree_json, l.vaccinated, l.state, l.description,
               l.batch_quantity, l.is_active, l.creation_timestamp, l.expiry_date, l.is_featured, l.is_verified_seller
        from public.listings l
        where l.breeder_id = r.user_id
