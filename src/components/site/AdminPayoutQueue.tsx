    ...
    <Card id="ready-for-payout" className="scroll-mt-6 overflow-hidden border-primary/30">
      <div className="border-b border-border bg-primary/5 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h2 className="flex items-center gap-2 text-xl font-semibold"><Banknote className="size-5 text-primary" /> READY FOR PAYOUT</h2><p className="mt-1 text-sm text-muted-foreground">Manually send the seller's payout through OPay first. This website never transfers the money.</p></div>
      ...
      <div className="divide-y divide-border">
        ...
        {!loading && ready.length === 0 ? <div className="p-6 text-center"><CheckCircle2 className="mx-auto size-8 text-primary" /><p className="mt-2 font-medium">No payouts waiting</p><p className="mt-1 text-sm text-muted-foreground">Transactions become eligible here only after receipt confirmation.</p></div> : ready.map((t) => {
      ...
      <div className="border-t border-border"><div className="flex items-center justify-between gap-3 p-5"><div><h3 className="font-semibold">Seller payout history</h3><p className="mt-1 text-xs text-muted-foreground">Recent manual payouts recorded after the OPay transfer was completed.</p></div><Badge variant="outline">{paid.length} recent</Badge></div>{paid.length === 0 ? <p className="px-5 pb-5 text-sm text-muted-foreground">No seller payouts recorded yet.</p> : <div className="divide-y divide-border">{paid.map((t) => { const seller = db.users.find((u) => u.id === t.breeder_id); const admin = db.users.find((u) => u.id === t.payout_paid_by); return <div key={t.id} className="grid gap-2 p-4 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="font-medium">{t.listing_name}</p><p className="break-all font-mono text-[11px] text-muted-foreground">Order {t.id}</p><p className="mt-1 text-xs text-muted-foreground">Seller: {seller?.real_name || seller?.public_handle || t.breeder_id} · Paid by: {admin?.real_name || admin?.public_handle || t.payout_paid_by}</p><p className="mt-1 text-xs text-muted-foreground">{t.payout_paid_at ? new Date(t.payout_paid_at).toLocaleString("en-NG") : "—"} · Ref: {t.payout_reference || "—"}</p></div><p className="text-lg font-bold text-primary">{ngn(Math.max(0, t.amount_naira - t.calculated_commission))}</p></div>; })}</div>}</div>
    </Card>
  );
}
