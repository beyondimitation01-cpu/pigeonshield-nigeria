...
function BreederDashboard() {
  ...
  function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    void addListing({ category_type: category, custom_bird_name: String(f.get("name") ?? ""), breed_type: breed, gender, price_ngn: Number(f.get("price") ?? 0), images: photos.map((p) => p.url), pedigree_json: null, vaccinated, state, description: String(f.get("description") ?? ""), batch_quantity: Number(f.get("qty") ?? 1) }).then(() => { form.reset(); setPhotos([]); toast.success("Listing published — live for 7 days."); }).catch((err: unknown) => { toast.error(err instanceof Error ? err.message : "Could not publish listing."); });
  }
  return (
...
function AccountPanel() {
  const { user, updateProfile } = useStore(); const [phone, setPhone] = useState(user?.phone_number ?? ""); const [fullName, setFullName] = useState(user?.real_name ?? ""); const [loft, setLoft] = useState(user?.loft_name ?? ""); const [homeState, setHomeState] = useState(user?.home_state ?? ""); const [saving, setSaving] = useState(false); const [deleting, setDeleting] = useState(false);
  useEffect(() => { if (!user) return; setPhone(user.phone_number); setFullName(user.real_name); setLoft(user.loft_name); setHomeState(user.home_state); }, [user]); if (!user) return null;
  async function saveProfile(e: React.FormEvent<HTMLFormElement>) { e.preventDefault(); const name = fullName.trim(); const normalizedPhone = phone.trim() ? formatNigerianPhone(phone) : ""; if (!name) { toast.error("Enter your full name."); return; } if (phone.trim() && !isValidNigerianPhone(phone)) { toast.error("Enter a valid Nigerian phone number."); return; } setSaving(true); const err = await updateProfile({ real_name: name, public_handle: name, phone_number: normalizedPhone, loft_name: loft.trim(), home_state: homeState }); setSaving(false); if (err) toast.error(err); else toast.success("Profile saved."); }
  async function deleteAccount() { setDeleting(true); try { const { error } = await supabase.rpc("delete_my_account"); if (error) throw new Error(error.message); await supabase.auth.signOut(); toast.success("Your account has been permanently deleted."); window.location.assign("/"); } catch (err) { toast.error(err instanceof Error ? err.message : "Could not delete your account. Your account was not changed."); } finally { setDeleting(false); } }
...
function ListingPhotoEditor({ listingId, userId, images }: { listingId: string; userId: string; images: string[] }) { const { setListingImages } = useStore(); const [photos, setPhotos] = useState<UploadedPhoto[]>(images.map((url) => ({ url, path: url }))); const [saving, setSaving] = useState(false); return <div className="space-y-2 rounded-md bg-muted/40 p-3"><PhotoUploader userId={userId} photos={photos} onChange={setPhotos} /><Button size="sm" disabled={saving} onClick={async () => { setSaving(true); const err = await setListingImages(listingId, photos.map((p) => p.url)); setSaving(false); if (err) toast.error(err); else toast.success("Listing photos saved."); }}>Save photos</Button></div>; }
function GuardedBreederDashboard() { return <ProtectedRoute title="Breeder Dashboard" description="Log in to manage your listings."><BreederDashboard /></ProtectedRoute>; }
