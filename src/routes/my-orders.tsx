...
function OrderCard({ tx, side, onChanged }: { tx: EscrowTransaction; side: "buyer" | "breeder"; onChanged: () => Promise<void> }) {
  ...
  return (
    <Card className="space-y-3 p-5">
      ...
      {side === "breeder" && tx.status === "Escrow Funded" ? <div className="rounded-md border border-primary/30 bg-primary/5 p-3"><p className="flex items-center gap-2 text-sm font-semibold text-primary"><Truck className="size-4" /> Ready for dispatch</p><p className="mt-1 text-xs text-muted-foreground">Dispatch the order to generate the seller handover PIN.</p><Button size="sm" className="mt-3" disabled={dispatching} onClick={async () => { setDispatching(true); try { await dispatchOrder(tx.id); toast.success("Order dispatched. Give the PIN to the driver."); await onChanged(); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not dispatch order."); } finally { setDispatching(false); } }}>{dispatching ? "Dispatching…" : "Dispatch order"}</Button></div> : null}
      ...
      {side === "buyer" && tx.status === "Escrow Funded" ? <div className="flex flex-wrap gap-2"><Button size="sm" variant="destructive" onClick={async () => { try { await reportDOA(tx.id, "doa_proof_video.mp4"); toast.error("DOA reported. Auto-release clock halted for admin review."); await onChanged(); } catch (error) { toast.error(error instanceof Error ? error.message : "Could not report DOA."); } }}><AlertTriangle className="size-4" /> Report Dead on Arrival</Button></div> : null}
...
function MyOrders() {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      const [{ data: activeRows, error: activeError }, { data: historyRows, error: historyError }] = await Promise.all([activeQuery, supabase.rpc("get_transaction_history")]);
      ...
      const active = ((activeRows ?? []) as Record<string, unknown>[]).map((row) => mapTransaction(row, pinMap.get(String(row["id"])) ?? null));
      const history = ((historyRows ?? []) as Record<string, unknown>[]).map((row) => mapTransaction(row));
      setPurchases(active.filter((tx) => tx.buyer_id === user.id)); setSales(active.filter((tx) => tx.breeder_id === user.id)); setPurchaseHistory(history.filter((tx) => tx.buyer_id === user.id)); setSalesHistory(history.filter((tx) => tx.breeder_id === user.id));
