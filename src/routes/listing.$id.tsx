    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          ...
          <Card className="space-y-1 bg-muted/50 p-4 text-sm"><p className="font-semibold text-foreground">Escrow breakdown</p><p className="text-muted-foreground">Platform commission: {pct}% {isUnitPriced ? "on the final purchase total" : `(${ngn((listing.price_ngn * pct) / 100)})`}</p>{isUnitPriced ? <p className="text-muted-foreground">The final breeder payout is calculated from the quantity purchased after commission.</p> : <p className="text-muted-foreground">Breeder receives on confirmation: {ngn(listing.price_ngn - (listing.price_ngn * pct) / 100)}</p>}</Card>
